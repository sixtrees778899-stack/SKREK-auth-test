(()=>{
  ...
  const deployment='legavik-customer-center-consistency-b11-20260916-1';
  ...
  const boot=({rootId,scripts,canonicalPath=location.pathname,bundleMarker,timeoutMs=30000})=>{
    ...
    if(root)root.dataset.runtimeState='VERSION_CHECK';let state='STARTING',timeout;
    ...
    const completeReady=()=>{if(state==='READY')return true;if(state!=='STARTING'||!identityMatches())return false;state='READY';clearTimeout(timeout);if(root){root.dataset.runtimeState='READY';root.dataset.startupState='READY';}return true;};
    const failStartup=(options={})=>{if(state==='READY'||identityReady()){completeReady();return false;}if(state!=='STARTING')return false;state='STARTUP_FAILED';clearTimeout(timeout);return renderFailure(root,{canonicalPath,...options});};
    if(root)startupControllers.set(root,Object.freeze({ready:completeReady,fail:failStartup,isReady:()=>state==='READY'||identityReady(),expectedDeployment:deployment}));
