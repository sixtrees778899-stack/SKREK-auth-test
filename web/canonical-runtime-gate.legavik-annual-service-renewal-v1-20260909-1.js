(()=>{
  ...
  const deployment='legavik-annual-service-renewal-v1-20260909-1';
  ...
  const readManifest=async()=>{const manifestUrl=localHarness?'/web/release-manifest.json':`${canonicalOrigin}${canonicalBase}/web/release-manifest.json`;const response=await fetch(`${manifestUrl}?ts=${Date.now()}`,{cache:'no-store'});if(!response.ok)throw new Error('manifest unavailable');return response.json();};
  ...
  const boot=({rootId,scripts,canonicalPath=location.pathname,bundleMarker,timeoutMs=12000})=>{
    ...
    const timeout=setTimeout(()=>{if(!finished)renderFailure(root,{canonicalPath});},timeoutMs);
    ...
    }catch{finished=true;clearTimeout(timeout);renderFailure(root,{canonicalPath});}})();return true;
  };
