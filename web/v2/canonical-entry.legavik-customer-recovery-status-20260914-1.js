(()=>{
  ...
  const appScript=pagesRuntime?'./v2-app.legavik-customer-recovery-status-20260914-1.bundle.js':'./v2-app.js';
  const approved=gate.boot({rootId:'app',canonicalPath:'/web/v2/index.html',bundleMarker:'recoveryMapBundleRelease',scripts:[{src:'../account/public-config.legavik-customer-recovery-status-20260914-1.js'},{src:'../account/account-nav-bridge.legavik-customer-recovery-status-20260914-1.bundle.js'},{src:appScript,type:'module',id:'recovery-map-app'}]});
  if(!approved)return;
  const renderFailure=()=>{if(!app||app.dataset.runtimeState!=='LOADING')return;app.dataset.runtimeState='FAILED';app.innerHTML='<section class="card initialization-failure" role="alert"><h1>Recovery Map 未能正常打开</h1><p>请重新打开当前页面。</p><div class="actions"><a class="button secondary" href="../account/index.html?section=maps&release=legavik-customer-recovery-status-20260914-1#center">返回客户中心</a></div></section>';};
