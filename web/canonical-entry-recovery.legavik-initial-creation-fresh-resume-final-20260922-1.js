(()=>{
  ...
  const query=new URLSearchParams(location.search),diagnosticRelease='legavik-initial-creation-fresh-resume-final-20260922-1',diagnosticMode=query.get('mode')==='version-update'&&query.get('version_number')==='4';
  if(diagnosticMode){
  ...
  const appScript=pagesRuntime?'./recover.legavik-initial-creation-fresh-resume-final-20260922-1.bundle.js':'./recover.js';
  const approved=gate.boot({rootId:'app',canonicalPath:'/web/recover.html',bundleMarker:'recoveryBundleRelease',scripts:[{src:appScript,type:'module',id:'recovery-app-script'}]});
  if(!approved)return;
  setTimeout(()=>{if(app?.dataset.runtimeState!=='LOADING')return;app.dataset.runtimeState='FAILED';app.innerHTML='<section class="card initialization-failure" role="alert"><h1>安全工作区未能正常打开</h1><p>请重新打开当前页面。</p><div class="actions"><a class="button secondary" href="./account/index.html?section=maps&release=legavik-initial-creation-fresh-resume-final-20260922-1#center">返回客户中心</a></div></section>';},12000);
})();
