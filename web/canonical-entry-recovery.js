(()=>{
  ...
  const query=new URLSearchParams(location.search),diagnosticRelease='final-ui-closure-20260905-1',diagnosticMode=query.get('mode')==='version-update'&&query.get('version_number')==='4';
  if(diagnosticMode){
  ...
  const appScript=pagesRuntime?'./recover.bundle.js?v=final-ui-closure-20260905-1':'./recover.js?v=final-ui-closure-20260905-1';
  const approved=gate.boot({rootId:'app',canonicalPath:'/web/recover.html?release=final-ui-closure-20260905-1',bundleMarker:'recoveryBundleRelease',scripts:[{src:appScript,type:'module',id:'recovery-app-script'}]});
  if(!approved)return;
  setTimeout(()=>{if(app?.dataset.runtimeState!=='LOADING')return;app.dataset.runtimeState='FAILED';app.innerHTML='<section class="card initialization-failure" role="alert"><h1>安全工作区未能正常打开</h1><p>请重新加载，或返回客户中心后再次进入。</p><div class="actions"><button id="reload-secure-workspace">重新加载</button><a class="button secondary" href="./account/index.html?section=maps&release=final-ui-closure-20260905-1#center">返回客户中心</a></div></section>';document.querySelector('#reload-secure-workspace')?.addEventListener('click',()=>location.reload());},12000);
})();
