(()=>{
  ...
  const approved=gate.boot({rootId:'account-app',canonicalPath:'/web/account/index.html',bundleMarker:'accountBundleRelease',scripts:[{src:'./public-config.legavik-annual-service-renewal-v1-20260909-1.js'},{src:'./account-nav-bridge.legavik-annual-service-renewal-v1-20260909-1.bundle.js'},{src:'./account-app.legavik-annual-service-renewal-v1-20260909-1.bundle.js',type:'module',id:'account-app-script'}]});
  if(!approved)return;
  let finished=false;
  const fail=()=>{if(finished||app?.dataset.runtimeState!=='LOADING')return;finished=true;app.dataset.runtimeState='FAILED';app.innerHTML='<main><section class="auth-layout"><div class="auth-card"><h1>客户中心未能正常打开</h1><p>请重新打开当前页面。</p><div class="auth-links"><a class="secondary-link" href="https://sixtrees778899-stack.github.io/SKREK-auth-test/web/v3-crypto/index.html?release=legavik-annual-service-renewal-v1-20260909-1#home">返回 LEGAVIK 首页</a></div></div></section></main>';};
