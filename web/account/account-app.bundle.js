...
function nextAction(data) {
  ...
  if (!latest) return { eyebrow: "START HERE", title: "\u521B\u5EFA\u7B2C\u4E00\u4EFD Recovery Map", text: "\u5148\u6574\u7406\u6700\u91CD\u8981\u7684\u8D44\u4EA7\u3001\u6062\u590D\u6761\u4EF6\u548C\u8D44\u6599\u4F4D\u7F6E\uFF0C\u5EFA\u7ACB\u6E05\u6670\u7684\u6062\u590D\u8D77\u70B9\u3002", href: recoveryMapCreateUrl, label: "\u5F00\u59CB\u5EFA\u7ACB" };
  if (/draft|incomplete|progress|未完成/i.test(latest.status || "")) return { eyebrow: "CONTINUE", title: "\u7EE7\u7EED\u672A\u5B8C\u6210\u7684 Recovery Map", text: "\u8FD4\u56DE\u5F53\u524D Recovery Map\uFF0C\u7EE7\u7EED\u5B8C\u6210\u5C1A\u672A\u6574\u7406\u7684\u6A21\u5757\u3002", href: recoveryMapCreateUrl, label: "\u7EE7\u7EED\u586B\u5199" };
...
function centerContent(data) {
  ...
  if (state.section === "overview") return `<div class="dashboard-heading"><p class="eyebrow">CUSTOMER DASHBOARD</p><h1>\u603B\u89C8</h1><p>\u5FEB\u901F\u67E5\u770B\u60A8\u7684\u6062\u590D\u51C6\u5907\u72B6\u6001\u4E0E\u4E0B\u4E00\u6B65\u3002</p></div><div class="stat-grid dashboard-stats"><article><span>Recovery Map \u72B6\u6001</span><strong>${latest ? "\u5F53\u524D\u7248\u672C" : "\u5C1A\u672A\u521B\u5EFA"}</strong><small>${latest ? `${currentVersionLabel(latest)} \xB7 ${displayDate(latest.published_at)}` : "\u4ECE\u7B2C\u4E00\u4EFD\u5730\u56FE\u5F00\u59CB"}</small></article><article><span>\u5F53\u524D\u670D\u52A1\u8BA1\u5212</span><strong>${esc2(plan)}</strong><small>${plan === "\u6682\u65E0" ? "\u5C1A\u65E0\u5DF2\u786E\u8BA4\u8BA1\u5212" : "\u5F53\u524D\u8D26\u6237\u670D\u52A1"}</small></article><article><span>Recovery Readiness / \u5E74\u5EA6\u68C0\u67E5</span><strong>${esc2(review?.status ?? "\u5C1A\u672A\u5F00\u59CB")}</strong><small>${review?.next_review_at ? `\u4E0B\u6B21\u5EFA\u8BAE ${displayDate(review.next_review_at)}` : "\u5C1A\u65E0\u68C0\u67E5\u65E5\u671F"}</small></article><article><span>\u6700\u8FD1\u6D3B\u52A8</span><strong>${esc2(activity?.label ?? "\u6682\u65E0\u6D3B\u52A8")}</strong><small>${displayDate(activity?.at, true)}</small></article></div><section class="next-action-card"><div><p class="eyebrow">${action.eyebrow}</p><h2>${action.title}</h2><p>${action.text}</p></div>${action.href ? `<a class="primary-link" href="${action.href}">${action.label}</a>` : `<button data-section-link="${action.section}">${action.label}</button>`}</section>${recentMaps.length ? `<section class="recent-maps"><header><div><p class="eyebrow">RECENT RECOVERY MAPS</p><h2>\u6700\u8FD1\u7684 Recovery Map</h2></div><button class="quiet-action" data-section-link="maps">\u67E5\u770B\u5168\u90E8</button></header><div class="recent-map-list">${recentMaps.map((item) => `<article><div><strong>${esc2(mapName(item))}</strong><span>\u5F53\u524D\u7248\u672C\uFF1A${currentVersionLabel(item)}</span></div><span class="status-pill">\u5F53\u524D\u7248\u672C</span><time datetime="${esc2(item.published_at)}">${displayDate(item.published_at, true)}</time><button class="quiet-action" data-section-link="maps">\u67E5\u770B Recovery Map \u2192</button></article>`).join("")}</div></section>` : ""}`;
