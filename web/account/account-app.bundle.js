...
var PostgrestFilterBuilder = class extends PostgrestTransformBuilder {
  ...
  */
  ...
  textSearch(column, query2, { config: config2, type } = {}) {
    let typePart = "";
  ...
  match(query2) {
    Object.entries(query2).filter(([_, value]) => value !== void 0).forEach(([column, value]) => {
      this.url.searchParams.append(column, `eq.${value}`);
    });
    return this;
  }
...
var Serializer = class {
  ...
  _encodeUserBroadcastPush(message2, encodingType, encodedPayload) {
    var _a, _b;
    const encoder = new TextEncoder();
    const topic = encoder.encode(message2.topic);
    const ref = encoder.encode((_a = message2.ref) !== null && _a !== void 0 ? _a : "");
    const joinRef = encoder.encode((_b = message2.join_ref) !== null && _b !== void 0 ? _b : "");
    const userEvent = encoder.encode(message2.payload.event);
...
var Channel = class {
  ...
  trigger(event, payload, ref, joinRef) {
    ...
    let eventBindings = this.bindings.filter((bind2) => bind2.event === event && this.filterBindings(bind2, payload, ref));
...
var Presence = class _Presence {
  ...
  static syncState(currentState, newState, onJoin, onLeave) {
    let state2 = this.toNullProtoObj(this.clone(currentState));
  ...
  static syncDiff(state2, diff, onJoin, onLeave) {
    state2 = this.toNullProtoObj(state2);
...
var RealtimeChannel = class _RealtimeChannel {
  ...
  _on(type, filter, callback) {
    ...
    if (typeLower === REALTIME_LISTEN_TYPES.POSTGRES_CHANGES) {
      const duplicate = (_a = this.bindings[typeLower]) === null || _a === void 0 ? void 0 : _a.find((bind2) => _RealtimeChannel.isSamePostgresFilter(bind2.filter, filter));
      if (duplicate) {
  ...
  _updateFilterMessage() {
    this.channelAdapter.updateFilterBindings((binding, payload, ref) => {
      ...
      const bind2 = (_a = this.bindings[typeLower]) === null || _a === void 0 ? void 0 : _a.find((bind3) => bind3.ref === binding.ref);
      if (!bind2) {
        return true;
      }
      if (["broadcast", "presence", "postgres_changes"].includes(typeLower)) {
        if ("id" in bind2) {
          ...
          const bindEvent = (_b = bind2.filter) === null || _b === void 0 ? void 0 : _b.event;
          ...
          const bindEvent = (_f = (_e = bind2 === null || bind2 === void 0 ? void 0 : bind2.filter) === null || _e === void 0 ? void 0 : _e.event) === null || _f === void 0 ? void 0 : _f.toLocaleLowerCase();
...
function createFetchClient(options) {
  ...
  return {
    async request({
      ...
      const url = buildUrl(options.baseUrl, path, query2);
...
var NamespaceOperations = class {
  ...
  async listNamespaces(parent) {
    const query2 = parent ? { parent: namespaceToPath(parent.namespace) } : void 0;
...
var StorageFileApi = class extends BaseApiClient {
  ...
  */
  async createSignedUrl(path, expiresIn, options) {
    ...
    return _this8.handleOperation(async () => {
      ...
      if (options === null || options === void 0 ? void 0 : options.download) query2.set("download", options.download === true ? "" : options.download);
      if ((options === null || options === void 0 ? void 0 : options.cacheNonce) != null) query2.set("cacheNonce", String(options.cacheNonce));
  ...
  */
  download(path, options, parameters) {
    ...
    const query2 = new URLSearchParams();
    if (options === null || options === void 0 ? void 0 : options.transform) this.applyTransformOptsToQuery(query2, options.transform);
    if ((options === null || options === void 0 ? void 0 : options.cacheNonce) != null) query2.set("cacheNonce", String(options.cacheNonce));
    const queryString = query2.toString();
  ...
  */
  async purgeCache(path, options, parameters) {
    ...
    return _this13.handleOperation(async () => {
      ...
      const query2 = new URLSearchParams();
      if (options === null || options === void 0 ? void 0 : options.transformations) query2.set("transformations", "true");
      const queryString = query2.toString();
  ...
  */
  ...
  applyTransformOptsToQuery(query2, transform) {
    if (transform.width) query2.set("width", transform.width.toString());
    if (transform.height) query2.set("height", transform.height.toString());
    if (transform.resize) query2.set("resize", transform.resize);
    if (transform.format) query2.set("format", transform.format);
    if (transform.quality) query2.set("quality", transform.quality.toString());
    return query2;
  }
};
...
var GoTrueClient = class _GoTrueClient {
  ...
  async _getUrlForProvider(url, provider, options) {
    ...
    if (options === null || options === void 0 ? void 0 : options.queryParams) {
      const query2 = new URLSearchParams(options.queryParams);
      urlParams.push(query2.toString());
    }
...
var PENDING_KEY = "skrek.customer-center.lifecycle-sync-pending.v1";
var client;
function getClient() {
  const config2 = globalThis.SKREK_PUBLIC_CONFIG ?? {};
  if (!config2.supabaseUrl || !config2.supabaseAnonKey) return null;
  client ??= createClient(config2.supabaseUrl, config2.supabaseAnonKey, { auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: false } });
  return client;
}
var txidPattern = /^[-_A-Za-z0-9]{43}$/;
var hashPattern = /^[0-9a-f]{64}$/;
function createPublishedLifecyclePayload({ operation, evidence, artifacts, displayName = "Recovery Map", plan = "\u672A\u6307\u5B9A", publishedAt } = {}) {
  ...
  return Object.freeze({
    ...
    p_published_at: publishedAt ?? evidence.first_downloadable_at ?? evidence.evidence_ready_at ?? (/* @__PURE__ */ new Date()).toISOString(),
    p_operation_state: "COMPLETE",
    p_evidence_status: "READY_FOR_INDEPENDENT_RECOVERY",
    p_verification_status: "PASS"
  });
}
async function syncPublishedRecoveryMapV1(input) {
  const payload = input?.p_operation_id ? input : createPublishedLifecyclePayload(input);
  const supabase2 = getClient();
  if (!supabase2) return { synced: false, reason: "NOT_CONFIGURED", payload };
  const { data: { session }, error: sessionError } = await supabase2.auth.getSession();
  if (sessionError) throw sessionError;
  if (!session) return { synced: false, reason: "NOT_AUTHENTICATED", payload };
  const { data, error } = await supabase2.rpc("record_published_recovery_map_v1", payload);
  if (error) throw error;
  return { synced: true, result: Array.isArray(data) ? data[0] : data, payload };
}
function pendingPublishedLifecycleSync({ storage = globalThis.localStorage } = {}) {
  try {
...
async function retryPendingPublishedLifecycleSync({ storage = globalThis.localStorage } = {}) {
  const payload = pendingPublishedLifecycleSync({ storage });
  if (!payload) return { synced: false, reason: "NOT_PENDING" };
  const result = await syncPublishedRecoveryMapV1(payload);
  if (result.synced) storage?.removeItem(PENDING_KEY);
  return result;
}
...
var supabase = configured ? createClient(config.supabaseUrl, config.supabaseAnonKey, {
...
var state = { view: "login", email: "", session: null, profile: null, section: "overview", cooldownUntil: 0 };
var esc2 = (value) => String(value ?? "").replace(/[&<>"']/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[char]);
document.addEventListener("click", (event) => {
  const menu = event.target.closest("#menu");
  if (!menu) return;
  const host = document.querySelector("#site-header"), open = host?.classList.toggle("menu-open") ?? false;
...
function authCard(title, intro, body) {
  return shell(`<section class="auth-layout"><div class="auth-card"><p class="eyebrow">SKREK ACCOUNT</p><h1>${title}</h1><p>${intro}</p>${body}</div></section>`);
}
...
function friendly(error) {
  const text = String(error?.message ?? error ?? "\u64CD\u4F5C\u672A\u5B8C\u6210\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002");
  if (/invalid login/i.test(text)) return "\u90AE\u7BB1\u6216\u5BC6\u7801\u4E0D\u6B63\u786E\u3002";
  if (/expired/i.test(text)) return "\u9A8C\u8BC1\u7801\u6216\u5B89\u5168\u94FE\u63A5\u5DF2\u8FC7\u671F\uFF0C\u8BF7\u91CD\u65B0\u83B7\u53D6\u3002";
  if (/token.*invalid|invalid.*token/i.test(text)) return "\u9A8C\u8BC1\u7801\u4E0D\u6B63\u786E\uFF0C\u8BF7\u68C0\u67E5\u540E\u91CD\u8BD5\u3002";
  if (/rate limit/i.test(text)) return "\u64CD\u4F5C\u8FC7\u4E8E\u9891\u7E41\uFF0C\u8BF7\u7A0D\u540E\u518D\u8BD5\u3002";
  if (/password/i.test(text) && /characters|weak/i.test(text)) return "\u5BC6\u7801\u81F3\u5C1110\u4F4D\uFF0C\u5E76\u5305\u542B\u5B57\u6BCD\u3001\u6570\u5B57\u548C\u7279\u6B8A\u5B57\u7B26\u3002";
...
function renderLogin() {
  ...
  bind("#forgot", "click", () => setView("forgot"));
  bind("#signup", "click", () => setView("signup"));
}
function renderSignup() {
  ...
  bind("#signup-form", "submit", async (event) => {
    ...
    button.disabled = true;
    const { data, error } = await supabase.auth.signUp({ email, password: temporaryPassword(), options: { emailRedirectTo: `${location.origin}${location.pathname}#verify` } });
    if (error) {
      button.disabled = false;
      showFormError(friendly(error));
      return;
    ...
    if (data.user && Array.isArray(data.user.identities) && data.user.identities.length === 0) {
      button.disabled = false;
      showFormError("\u8BE5\u90AE\u7BB1\u5DF2\u6CE8\u518C\uFF0C\u8BF7\u76F4\u63A5\u767B\u5F55\u6216\u91CD\u7F6E\u5BC6\u7801\u3002");
      return;
    }
    state.email = email;
  ...
  bind("#login", "click", () => setView("login"));
}
function renderVerify() {
  ...
  bind("#verify-form", "submit", async (event) => {
    ...
    const button = event.submitter;
    button.disabled = true;
    const { data, error } = await supabase.auth.verifyOtp({ email: state.email, token: document.querySelector("#otp").value, type: "signup" });
    if (error) {
      button.disabled = false;
      showFormError(friendly(error));
      return;
  ...
  bind("#resend", "click", async () => {
    ...
    const { error } = await supabase.auth.resend({ type: "signup", email: state.email, options: { emailRedirectTo: `${location.origin}${location.pathname}#verify` } });
    if (error) return showFormError(friendly(error));
    state.cooldownUntil = Date.now() + 6e4;
    showFormError("\u9A8C\u8BC1\u7801\u5DF2\u91CD\u65B0\u53D1\u9001\u3002");
  });
  bind("#change", "click", () => setView("signup"));
}
function passwordForm(title, intro, mode) {
  const username = mode === "signup" ? '<label>\u7528\u6237\u540D<input id="username" autocomplete="username" minlength="3" maxlength="30" pattern="[A-Za-z0-9_]+" required></label><small>3\u201330\u4F4D\u82F1\u6587\u5B57\u6BCD\u3001\u6570\u5B57\u6216\u4E0B\u5212\u7EBF\uFF1B\u7528\u6237\u540D\u5168\u5E73\u53F0\u552F\u4E00\u3002</small>' : "";
  app.innerHTML = authCard(title, intro, `<form id="password-form">${username}<label>\u65B0\u5BC6\u7801<input id="password" type="password" autocomplete="new-password" required></label><small>\u81F3\u5C1110\u4F4D\uFF0C\u5305\u542B\u5B57\u6BCD\u3001\u6570\u5B57\u548C\u7279\u6B8A\u5B57\u7B26\u3002</small><label>\u786E\u8BA4\u5BC6\u7801<input id="confirm" type="password" autocomplete="new-password" required></label><div id="form-message"></div><button type="submit">\u4FDD\u5B58\u5E76\u5B8C\u6210</button></form>`);
  bind("#password-form", "submit", async (event) => {
    ...
    button.disabled = true;
    if (mode === "signup") {
      const desired_username = document.querySelector("#username").value.trim();
      const { error: usernameError } = await supabase.rpc("claim_username", { desired_username });
      if (usernameError) {
        button.disabled = false;
        showFormError(friendly(usernameError));
        return;
      }
    }
    const metadata = mode === "signup" ? { username: document.querySelector("#username").value.trim() } : void 0;
    const { error } = await supabase.auth.updateUser({ password, ...metadata ? { data: metadata } : {} });
    if (error) {
      button.disabled = false;
      showFormError(friendly(error));
      return;
    }
    if (mode === "reset") await supabase.auth.signOut();
    else state.profile = await ensureProfile();
    mode === "reset" ? setView("login") : await loadSession();
  });
}
function renderForgot() {
  ...
  bind("#forgot-form", "submit", async (event) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(document.querySelector("#email").value.trim(), { redirectTo });
    if (error) {
      button.disabled = false;
      showFormError(friendly(error));
      return;
    }
    document.querySelector("#form-message").outerHTML = message("\u91CD\u7F6E\u90AE\u4EF6\u5DF2\u53D1\u9001\uFF0C\u8BF7\u68C0\u67E5\u90AE\u7BB1\u3002", "success");
  });
  bind("#login", "click", () => setView("login"));
}
...
async function fetchCenter() {
  ...
  const byMap = versions.data.reduce((result, item) => {
    ...
    history2.push(item);
    result.set(item.recovery_map_id, history2);
    return result;
  }, /* @__PURE__ */ new Map()), formalMaps = maps.data.map((item) => {
...
function empty(text, button = "") {
  return `<div class="empty-state"><p>${text}</p>${button}</div>`;
}
function displayDate(value, withTime = false) {
  if (!value) return "\u5C1A\u65E0\u8BB0\u5F55";
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? "\u5C1A\u65E0\u8BB0\u5F55" : withTime ? date.toLocaleString("zh-CN") : date.toLocaleDateString("zh-CN");
}
function recentActivity(data) {
  const entries = [...data.maps.map((item) => ({ label: "Recovery Map \u5DF2\u66F4\u65B0", at: item.updated_at || item.created_at })), ...data.reviews.map((item) => ({ label: "\u5E74\u5EA6\u68C0\u67E5\u72B6\u6001\u5DF2\u66F4\u65B0", at: item.last_reviewed_at || item.created_at })), ...data.orders.map((item) => ({ label: "\u8BA2\u5355\u72B6\u6001\u5DF2\u66F4\u65B0", at: item.updated_at || item.created_at }))].filter((item) => item.at).sort((left, right) => new Date(right.at) - new Date(left.at));
  return entries[0] || null;
}
function mapName(item) {
  return item.display_name || `Recovery Map ${item.recovery_map_id.slice(-8)}`;
}
function currentVersionLabel(item) {
  return item.current_version ? `V${item.current_version.version_number}` : "\u5C1A\u65E0\u5DF2\u53D1\u5E03\u7248\u672C";
}
function versionHistory(item) {
  return `<details class="version-history"><summary>\u67E5\u770B\u7248\u672C\u8BB0\u5F55</summary><div>${item.versions.map((version5) => `<article><div><strong>V${version5.version_number}</strong><span>${version5.status === "CURRENT" ? "\u5F53\u524D\u7248\u672C" : "\u5386\u53F2\u7248\u672C"}</span></div><time datetime="${esc2(version5.published_at)}">${displayDate(version5.published_at, true)}</time></article>`).join("")}</div></details>`;
}
function nextAction(data) {
  const latest = data.maps[0], review = data.reviews[0];
  if (!latest) return { eyebrow: "START HERE", title: "\u521B\u5EFA\u7B2C\u4E00\u4EFD Recovery Map", text: "\u5148\u6574\u7406\u6700\u91CD\u8981\u7684\u8D44\u4EA7\u3001\u6062\u590D\u6761\u4EF6\u548C\u8D44\u6599\u4F4D\u7F6E\uFF0C\u5EFA\u7ACB\u6E05\u6670\u7684\u6062\u590D\u8D77\u70B9\u3002", href: "../v2/index.html?entry=guide#guide", label: "\u5F00\u59CB\u5EFA\u7ACB" };
  if (/draft|incomplete|progress|未完成/i.test(latest.status || "")) return { eyebrow: "CONTINUE", title: "\u7EE7\u7EED\u672A\u5B8C\u6210\u7684 Recovery Map", text: "\u8FD4\u56DE\u5F53\u524D Recovery Map\uFF0C\u7EE7\u7EED\u5B8C\u6210\u5C1A\u672A\u6574\u7406\u7684\u6A21\u5757\u3002", href: "../v2/index.html?entry=guide#guide", label: "\u7EE7\u7EED\u586B\u5199" };
  if (review?.next_review_at && new Date(review.next_review_at) <= /* @__PURE__ */ new Date()) return { eyebrow: "RECOVERY READINESS", title: "\u5B8C\u6210\u5E74\u5EA6\u68C0\u67E5", text: "\u68C0\u67E5\u8D44\u4EA7\u3001\u8BBE\u5907\u3001\u8BA4\u8BC1\u65B9\u5F0F\u548C\u6062\u590D\u8D44\u6599\u72B6\u6001\u662F\u5426\u4ECD\u7136\u51C6\u786E\u3002", section: "reviews", label: "\u67E5\u770B\u5E74\u5EA6\u68C0\u67E5" };
  if (!data.materials.length) return { eyebrow: "RECOVERY MATERIALS", title: "\u786E\u8BA4\u6062\u590D\u8D44\u6599\u72B6\u6001", text: "\u6838\u5BF9\u5F53\u524D\u7248\u672C\u6240\u9700\u6062\u590D\u8D44\u6599\u662F\u5426\u5DF2\u6309\u65E2\u5B9A\u5B89\u6392\u4FDD\u5B58\u5E76\u53EF\u5728\u9700\u8981\u65F6\u53D6\u5F97\u3002", section: "materials", label: "\u67E5\u770B\u8D44\u6599\u72B6\u6001" };
  return { eyebrow: "KEEP IT CURRENT", title: "\u68C0\u67E5\u6700\u8FD1\u7684 Recovery Map", text: "\u786E\u8BA4\u6700\u8FD1\u7248\u672C\u4ECD\u7136\u53CD\u6620\u60A8\u5F53\u524D\u4F7F\u7528\u7684\u8D44\u4EA7\u3001\u8BBE\u5907\u548C\u6062\u590D\u6761\u4EF6\u3002", section: "maps", label: "\u67E5\u770B Recovery Map" };
}
function centerContent(data) {
  const latest = data.maps[0], plan = data.orders.find((item) => item.payment_status === "paid" || item.payment_status === "PAID")?.plan || latest?.plan || "\u6682\u65E0", review = data.reviews[0], activity = recentActivity(data), action = nextAction(data), recentMaps = data.maps.slice(0, 3);
  if (state.section === "overview") return `<div class="dashboard-heading"><p class="eyebrow">CUSTOMER DASHBOARD</p><h1>\u603B\u89C8</h1><p>\u5FEB\u901F\u67E5\u770B\u60A8\u7684\u6062\u590D\u51C6\u5907\u72B6\u6001\u4E0E\u4E0B\u4E00\u6B65\u3002</p></div><div class="stat-grid dashboard-stats"><article><span>Recovery Map \u72B6\u6001</span><strong>${latest ? "\u5F53\u524D\u7248\u672C" : "\u5C1A\u672A\u521B\u5EFA"}</strong><small>${latest ? `${currentVersionLabel(latest)} \xB7 ${displayDate(latest.published_at)}` : "\u4ECE\u7B2C\u4E00\u4EFD\u5730\u56FE\u5F00\u59CB"}</small></article><article><span>\u5F53\u524D\u670D\u52A1\u8BA1\u5212</span><strong>${esc2(plan)}</strong><small>${plan === "\u6682\u65E0" ? "\u5C1A\u65E0\u5DF2\u786E\u8BA4\u8BA1\u5212" : "\u5F53\u524D\u8D26\u6237\u670D\u52A1"}</small></article><article><span>Recovery Readiness / \u5E74\u5EA6\u68C0\u67E5</span><strong>${esc2(review?.status ?? "\u5C1A\u672A\u5F00\u59CB")}</strong><small>${review?.next_review_at ? `\u4E0B\u6B21\u5EFA\u8BAE ${displayDate(review.next_review_at)}` : "\u5C1A\u65E0\u68C0\u67E5\u65E5\u671F"}</small></article><article><span>\u6700\u8FD1\u6D3B\u52A8</span><strong>${esc2(activity?.label ?? "\u6682\u65E0\u6D3B\u52A8")}</strong><small>${displayDate(activity?.at, true)}</small></article></div><section class="next-action-card"><div><p class="eyebrow">${action.eyebrow}</p><h2>${action.title}</h2><p>${action.text}</p></div>${action.href ? `<a class="primary-link" href="${action.href}">${action.label}</a>` : `<button data-section-link="${action.section}">${action.label}</button>`}</section><section class="recent-maps"><header><div><p class="eyebrow">RECENT RECOVERY MAPS</p><h2>\u6700\u8FD1\u7684 Recovery Map</h2></div>${recentMaps.length ? '<button class="quiet-action" data-section-link="maps">\u67E5\u770B\u5168\u90E8</button>' : ""}</header>${recentMaps.length ? `<div class="recent-map-list">${recentMaps.map((item) => `<article><div><strong>${esc2(mapName(item))}</strong><span>\u5F53\u524D\u7248\u672C\uFF1A${currentVersionLabel(item)}</span></div><span class="status-pill">\u5F53\u524D\u7248\u672C</span><time datetime="${esc2(item.published_at)}">${displayDate(item.published_at, true)}</time><button class="quiet-action" data-section-link="maps">\u67E5\u770B Recovery Map \u2192</button></article>`).join("")}</div>` : empty("\u521B\u5EFA Recovery Map \u540E\uFF0C\u6700\u8FD1\u7248\u672C\u4F1A\u663E\u793A\u5728\u8FD9\u91CC\u3002")}</section>`;
  if (state.section === "maps") return `<div class="dashboard-heading"><p class="eyebrow">RECOVERY MAP LIFECYCLE</p><h1>\u6211\u7684 Recovery Map</h1><p>\u8FD9\u91CC\u53EA\u663E\u793A\u5DF2\u6210\u529F\u53D1\u5E03\u7684\u7248\u672C\u8BB0\u5F55\uFF0C\u4E0D\u4FDD\u5B58 Recovery Map \u5185\u5BB9\u6216\u6062\u590D\u79D8\u5BC6\u3002</p></div>${data.maps.length ? `<div class="record-list lifecycle-map-list">${data.maps.map((item) => `<article><header><div><h2>${esc2(mapName(item))}</h2><p>\u5F53\u524D\u7248\u672C\uFF1A<strong>${currentVersionLabel(item)}</strong></p></div><span class="status-pill">\u5F53\u524D\u7248\u672C</span></header><p>\u521B\u5EFA\u65F6\u95F4\uFF1A${displayDate(item.published_at, true)}</p><button class="quiet-action" data-version-info="${esc2(item.id)}">\u67E5\u770B Recovery Map \u7248\u672C\u4FE1\u606F</button>${versionHistory(item)}</article>`).join("")}</div>` : empty("\u60A8\u8FD8\u6CA1\u6709\u5DF2\u6210\u529F\u53D1\u5E03\u7684 Recovery Map\u3002")}`;
  if (state.section === "materials") return `<h1>\u6062\u590D\u8D44\u6599\u72B6\u6001</h1><p class="security-note">\u8FD9\u91CC\u53EA\u663E\u793A\u975E\u654F\u611F\u72B6\u6001\u4E0E\u6587\u4EF6\u6807\u8BC6\u3002SKREK \u4E0D\u4FDD\u5B58 Recovery Password\u3001Data Key\u3001Recovery Kit \u6216 Mainnet Recovery Evidence \u6587\u4EF6\u5185\u5BB9\uFF1B\u539F\u59CB\u6062\u590D\u6750\u6599\u7531\u60A8\u81EA\u884C\u4FDD\u7BA1\u3002</p>${data.materials.length ? `<div class="record-list">${data.materials.map((item) => `<article><h2>${esc2(item.filename)}</h2><p>${esc2(item.material_type)} \xB7 ${new Date(item.created_at).toLocaleString("zh-CN")}</p><span>\u5BA2\u6237\u81EA\u884C\u4FDD\u7BA1\uFF0C\u5BA2\u6237\u4E2D\u5FC3\u4E0D\u63D0\u4F9B\u91CD\u65B0\u4E0B\u8F7D</span></article>`).join("")}</div>` : empty("\u5F53\u524D\u6CA1\u6709\u6062\u590D\u8D44\u6599\u72B6\u6001\u8BB0\u5F55")}`;
...
async function renderCenter() {
  ...
  try {
    ...
    document.querySelectorAll("[data-section],[data-section-link]").forEach((button) => button.onclick = () => {
      state.section = button.dataset.section || button.dataset.sectionLink;
      renderCenter();
    });
    bind("#logout", "click", async () => {
      if (!lifecyclePreview) await supabase.auth.signOut();
      state.session = null;
      state.profile = null;
      setView("login");
    });
    bind("#change-password", "click", () => passwordForm("\u4FEE\u6539\u767B\u5F55\u5BC6\u7801", "\u8BBE\u7F6E\u65B0\u7684 SKREK \u767B\u5F55\u5BC6\u7801\u3002", "change"));
    bind("#forgot-password", "click", () => setView("forgot"));
    ...
    app.innerHTML = authCard("\u5BA2\u6237\u4E2D\u5FC3\u6682\u65F6\u65E0\u6CD5\u8BFB\u53D6", friendly(error), '<button id="retry">\u91CD\u8BD5</button>');
    bind("#retry", "click", renderCenter);
  }
}
...
async function loadSession() {
  ...
  if (session) {
    if (requested === "reset-password") return passwordForm("\u91CD\u7F6E\u767B\u5F55\u5BC6\u7801", "\u8BF7\u8BBE\u7F6E\u65B0\u7684 SKREK \u767B\u5F55\u5BC6\u7801\u3002", "reset");
    state.profile = await ensureProfile();
    await retryPendingPublishedLifecycleSync().catch(() => {
    });
    if (purchase.active) {
      state.view = "payment-pending";
      return renderPaymentPending();
    }
    state.view = "center";
    await renderCenter();
  } else {
    state.view = ["signup", "forgot", "login"].includes(requested) ? requested : purchase.active ? "signup" : "login";
    render();
  }
}
function render() {
  ...
  if (state.session || state.view === "center") return renderCenter();
  if (state.view === "signup") return renderSignup();
  if (state.view === "verify") return renderVerify();
  if (state.view === "set-password") return passwordForm("\u8BBE\u7F6E\u767B\u5F55\u5BC6\u7801", "\u90AE\u7BB1\u9A8C\u8BC1\u6210\u529F\u3002\u8BF7\u8BBE\u7F6E\u6B63\u5F0F\u767B\u5F55\u5BC6\u7801\u3002", "signup");
  if (state.view === "forgot") return renderForgot();
  if (state.view === "reset-password") return passwordForm("\u8BBE\u7F6E\u65B0\u5BC6\u7801", "\u8BF7\u8BBE\u7F6E\u65B0\u7684 SKREK \u767B\u5F55\u5BC6\u7801\u3002", "reset");
  return renderLogin();
}
if (configured) {
  supabase.auth.onAuthStateChange((event, session) => {
...
state.view = location.hash.slice(1) || "login";
await loadSession();
